# GATE — living8 ICQR door (GLB mini cabinets, 0.5/1/0.5 lights)

Phone-width (390×844). `app.js?v=living8`.
...
SoT: **ICQR-first door** — pixelated QR field of miniature PORTABOOM cabinets cloned from the real GLB cabinet mesh, planted unit (cabinet + traffic light + **one** boom) **straight-on** so lenses read, big PORTABOOM logo in the back. Then green 0.5s → amber 1s → red 0.5s → boom down. Then DEST. **Not** a twin-site 3/4 plaza. **Not** WordPress.
...
| Check | Result |
| --- | --- |
| `?showtime=1` first paint is ICQR QR field | GREEN — `viewMode=door`, `showtimePhase=door`, cream QR paper, mini cabinets from the GLB, **not** living2 `lockWorldCamera` 3/4 plaza |
...
| Timing ~0.5 / 1.0 / 0.5 then boom | GREEN — measured green **0.506s** → amber **1.207s** → red hold **0.607s** → boom down **0.896s**; settle **3.216s**. Then **0.4s** beat and `location.assign(leaveDest)` |
...
| Stationary send QR | jsQR decodes living showtime URL `?v=living8&showtime=1` from `fabian-showtime-qr.png` (not DEST) |
...
| `?v=living8&showtime=1` | ICQR door → 0.5/1/0.5 + boom → default DEST. Query `showtime=1` is the switch |
...
| Claim | Refute |
| --- | --- |
| Opens looking like another website / twin hub | `showtime-door.png` is the QR matrix + unit. Studio cyclorama off. HUD/brand gone. `product=living8-icqr-door` |
...
| Field is black blocks + stripe towers + mini TLs | `dressMinisFromCabinet` instances the real cabinet mesh. No lens geos. `miniHasTrafficLight=false` `miniFromGlb=true` |
| Minis are abstract boxes / navy-band junk | Scaled GLB cabinet clones (`MiniCabinetField`). living7 box heap rejected |
...
2. Camera opens the living Incredible QR **door** — straight-on cabinet, traffic light lenses readable, one boom, mini PORTABOOMs cloned from the real cabinet, brand in the back.
