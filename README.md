...
- **`?showtime=1`** (and `?v=living3d&showtime=1`) auto-plays ~7s — longer than the living2 teaser GIF loop (~3.6s). Clear amber, then red as the boom lowers. After the boom is down, the page goes to DEST.
...
- The scannable matrix lives in the terrain. Tap / **Tap to scan** for a clearer scan pose (baked default product URL).
...
Cache-bust: `app.js?v=living3d`

Pages: https://tas-fdnxt.github.io/portaboom-incredible-qr/

Send QR (after Chief merge): `fabian-showtime-qr.png` → `https://tas-fdnxt.github.io/portaboom-incredible-qr/?v=living3d&showtime=1`

## How to scan

1. Phone camera on the stationary send QR opens this living page with showtime.
2. After the boom drop, the page automatically opens DEST (default product page, or `?dest=` if set).
